...
def optimium_infer(input_image: Image, model_path: Path):
    ...
    input_shape = model.get_input_tensor_info(
        0
    ).shape  # Face Detection has only one input (NHWC)
    height, width = input_shape[1:3]

    # Preprocess input image
    image_data = preprocess_image(height, width, input_image)
...
def post_process_inference_to_dection(
    info_for_postprocess, output_data: List[npt.NDArray]
) -> List[Detection]:
    ...
    pruned_detections = non_maximum_suppression(
        detections, MIN_SUPPRESSION_THRESHOLD, MIN_SCORE, weighted=True
    )
...
def face_detection(image_path: Path, model_path: Path, output_path: Path):
    ...
    detected_faces = post_process_inference_to_dection(
        info_for_postprocess, output_data
    )
    ...
    if not len(detected_faces):
        ...
        render_data = detections_to_render_data(
            detected_faces, bounds_color=Colors.GREEN, line_width=5
        )
...
if __name__ == "__main__":
    ...
    face_detection(
        image_path=image_path, model_path=model_path, output_path=output_path
    )
